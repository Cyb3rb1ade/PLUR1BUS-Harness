...
- **Policy catalogue:** every profile carries `policy_status`/`policy_source`/`policy_checked`; **Anthropic via Claude Code / Agent SDK** ships `allowed` (spawns the unmodified binary/SDK under the user's own login, never a harness-native Claude.ai login); a harness-native Anthropic OAuth stays absent, as does Google subscription OAuth; OpenAI and xAI are **not shipped**; the absent/not-shipped ones render as visible **disabled** entries with the reason and the ACP alternative. `provider-matrix.md` §4; ADR-005 §"Conflicts".

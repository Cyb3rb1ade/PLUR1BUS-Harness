...
- **Policy catalogue:** every profile carries `policy_status`/`policy_source`/`policy_checked`; Anthropic/Google subscription OAuth **absent**, OpenAI and xAI **not shipped**, both rendered as visible **disabled** entries with the reason and the ACP alternative. `provider-matrix.md` §4; ADR-005 §"Conflicts".
...
1. **Device-code login headless over SSH** succeeds for a profile that documents one; where none exists, the loopback+`ssh -L` and paste-callback paths succeed instead and the CLI says which was used (§12 M2, re-cut because no frontier vendor documents a usable device code — ADR-005).
...
| R4 | **Vendor policy changes** — Anthropic/Google `prohibited`, OpenAI ambiguous, xAI unverified; rules moved several times in 2026 | 3 | 4 | Policy is **data**: `policy_status`/`policy_source`/`policy_checked` per profile; CI refuses to load a `prohibited` profile; dated re-check every 90 days (first 2026-12-21); compliant substitutes (API keys, vendor CLI over ACP) shipped | Owner | A vendor announcement, or a re-check finding a changed status |
...
| **Subscription OAuth for Anthropic, Google, OpenAI, xAI** — not as opt-in, not behind a flag; API keys and the vendor CLI over ACP instead | ADR-005 §"Conflicts"; `provider-matrix.md` §4 |

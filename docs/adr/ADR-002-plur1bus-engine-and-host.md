...
|---|---|---|---|
| Recall hook envelope | **50 000 ms** (`recallTimeoutMs` 45 000 + 5 000, `index.js:13351`; `lib/runtime-scheduler.js:7,40`) | **hard 600 ms** (re-answered 2026-09-22, owner verbatim "40/60" — coordinator reads this as 400/600 ms, tightened from this ADR's original 1 200 ms; **interpretation pending owner confirmation**) | The 45 s value exists to survive a host that aborts at 15 s (comment `index.js:12311-12313`). We own the caller. D6 requires recall inside a budget *in parallel with prompt assembly* and first token streamed immediately; a 45 s recall is incompatible with that. Whichever exact number is confirmed, measuring today's pipeline before PR-01 stays mandatory. |
| Recall soft budget | 35 000 ms (`index.js:4711`), scheduler-derived `max(1000, min(hard−2000, hard×0.5))` (`lib/runtime-scheduler.js:392`) | **soft 400 ms** (unchanged by the 2026-09-22 answer) | Same derivation shape (≈⅓ of hard), absolute value set by the TTFT target. Past soft, the engine returns what it has. |
...
1. **Q1 — answered 2026-09-22 (owner confirmed "yes", B7).** Package names as proposed: `@cyb3rb1ade/plur1bus-engine`, `…-host-openclaw`, `…-control-ui`, `…-platform`; `@cyb3rb1ade/plur1bus-memory` stays the **adapter**, so existing OpenClaw installs are unaffected.
2. **Q2 — re-answered 2026-09-22, owner verbatim: "40/60."** Coordinator's interpretation: **soft 400 ms / hard 600 ms** (tighter than this ADR's original 400/1 200 proposal). **This interpretation is pending owner confirmation** — "40/60" is terse enough to admit other readings. Either way, the mandatory measurement of today's pipeline (p50/p95/p99 against the phase timer already in the code) before PR-01 stays required and may argue for a different number once real data exists.

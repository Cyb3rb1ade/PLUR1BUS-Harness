...
| §6.2 engine events forwarded as notifications named after the event | One `engine.event { name, agentId?, payload }` notification | Keeps the notification set closed in the schema while the engine's event list grows; the payload is still verbatim.  **Superseded by ADR-016 §6** (harness-owned event schemas in H2; `engine.event` is experimental until then). |

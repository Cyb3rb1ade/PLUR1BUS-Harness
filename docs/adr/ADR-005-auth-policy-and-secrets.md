...
1. **§13 Q3 — answered here, confirm:** recommendation is *opt-in with a visible risk notice remains the policy for `restricted`, but no `restricted` profile ships at v0.1, and no `prohibited` profile ships ever.* Confirm this replaces the original's "Default: Opt-in"; `docs/assumptions.md` Q3 already carries the superseded-by-evidence wording and awaits this confirmation.

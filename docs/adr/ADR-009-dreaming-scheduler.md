...
**The harness owns the dreaming scheduler. It never uses a host cron, an `agentTurn` system event, or an external provisioning script.** The scheduler lives in the resident core daemon (ADR-001) and drives a **job registry fed by the PLUR1BUS engine** (`engine.jobs`, ADR-002 P5), which is the sole owner of the dreaming logic (D4, §2.1 "Doppelte Lernschleifen vermeiden"). Every run — including every skip — writes one durable record before any early return. Three phases are first-class objects with their own schedule, enable switch, status, log and "run now". Triggering is **importance-accumulation primary, cron as a floor.** Guards are mandatory, not optional: dedupe, a hard circuit breaker on session count, stale-candidate expiry, minimum corpus, per-agent cost **measurement** (no fixed spend cap in M1 — see the B5/Q1 amendment below). Model-free jobs stay model-free; LLM-using jobs run on the agent's own model and credentials, never hardwired (§4.1). **Amended 2026-09-22 (owner decision D15):** the deep phase promotes into **`memories.md`** (consolidated long-term facts) and **`knowledgepool.md`** (knowledge-pool content — `knowledge_update` output and REM/consolidation summaries), light dreaming's shortlist feeds **`DailyNote_YYYY-MM-DD_HHMMSS.md`**, and the diary is **`dreaming.md`** — four required per-agent curated files, superseding the earlier `KNOWLEDGE.md`/`DREAMS.md` default (see "### Amendment D15" below).
...
|---|---|---|
| **Circuit breaker** | **Re-answered 2026-09-22 (owner decision, B5): "no - 1st just measure it."** Per agent per sweep: hard breach on **session count only** — max **3 LLM sessions** aborts the sweep, sets `breaker_state='open'` until the next scheduled window, surfaces a visible error. **No fixed cost/spend cap ships in M1.** Instead, M1 records cost per phase per run (`dream_run.cost_micros`) in the ledger so a cap can be set from real numbers once measured, rather than guessed | #65550: 94 sessions / 65 min / $4.35, "no rate limit or circuit breaker" (session-count breaker is the direct regression test, cost is observed not capped) |
...
### Amendment D15 (2026-09-22): four curated memory files, not `KNOWLEDGE.md`/`DREAMS.md`

Owner answer to A5, verbatim: *"DailyNote_YYYY-MM-DD_HHMMSS.md (o.ä.), memories.md, dreaming.md, und knowledgepool.md brauchen wir alles."* This supersedes the Q10 default (`memory/KNOWLEDGE.md` for promotions, `DREAMS.md` for the diary) with **four required per-agent curated files**:

| File | Role | Fed by |
|---|---|---|
| `DailyNote_YYYY-MM-DD_HHMMSS.md` | Timestamped daily notes — the light-sleep phase's shortlist/working layer. Naming may still be adjusted ("o.ä.") | Light phase (`classify-recent`, `afterthought` triage) |
| `memories.md` | Long-term memory — the deep-sleep **promotion target** for consolidated, durable facts | Deep phase promotion (this ADR's `dream_candidate` → promoted path) |
| `dreaming.md` | The dream diary | Deep phase, one entry per sweep plus phase summaries (unchanged in mechanics from the "dream diary" row above, only the filename changes) |
| `knowledgepool.md` | The curated knowledge corpus | REM/consolidation summaries and `knowledge_update` content (the tool named in the original commission's hook list, auftrag §4.1) |

**Proposal, not yet implementation-tested:** the `memories.md`/`knowledgepool.md` split is this ADR's reading of "long-term memory" vs. "knowledge pool" — memories.md receives promoted engrams (facts, decisions, durable non-profile content, the direct analogue of OpenClaw's `MEMORY.md`/PLUR1BUS's old `KNOWLEDGE.md` promotion target), knowledgepool.md receives broader knowledge-corpus material (semantic-link discoveries, REM pattern/trend summaries, `knowledge_update` tool output). This split is marked a proposal because Phase 0 has no evidence yet on where the line best falls; the M1 implementation should treat it as adjustable pending real dreaming output.

**What does not change:** the engine's OpenClaw adapter keeps writing OpenClaw's own `KNOWLEDGE.md`/`DREAMS.md` filenames for OpenClaw hosts — D15 is a **harness-host** mapping only, so the OpenClaw plugin's existing users see no file-name change. The importer (`docs/import.md`) maps OpenClaw's `MEMORY.md` → `memories.md`, `memory/YYYY-MM-DD.md` → `DailyNote_*`, `KNOWLEDGE.md` → `knowledgepool.md`, `DREAMS.md` → `dreaming.md` (reference-only); Hermes's `MEMORY.md` → `memories.md`.

## Open questions for the owner

1. **Q1 — default cost cap. Answered 2026-09-22: "no - 1st just measure it."** No fixed cap ships in M1; the session-count circuit breaker (3/sweep) is the only hard abort, and cost per phase per run is measured in the ledger. A cap (dollar or token) is set from that data in a later milestone.
...
3. **Q3 — promotion target. Re-answered 2026-09-22 (owner decision D15), verbatim: "DailyNote_YYYY-MM-DD_HHMMSS.md (o.ä.), memories.md, dreaming.md, und knowledgepool.md brauchen wir alles."** Supersedes the `KNOWLEDGE.md` default below. Four required per-agent curated files: `DailyNote_YYYY-MM-DD_HHMMSS.md` (daily notes, light-sleep input), `memories.md` (long-term memory, the deep-sleep promotion target for consolidated facts), `dreaming.md` (the dream diary), `knowledgepool.md` (the knowledge pool — `knowledge_update` content and REM/consolidation summaries). See "### Amendment D15" below for the split rationale, marked a **proposal**. *(Original question, superseded: OpenClaw's deep phase promotes into `MEMORY.md`, which does not exist anywhere in the PLUR1BUS codebase; PLUR1BUS's own curated file was `memory/KNOWLEDGE.md`.)*

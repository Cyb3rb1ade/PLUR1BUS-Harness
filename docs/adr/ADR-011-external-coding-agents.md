...
- **Vendor policy makes "run the vendor's own CLI" the only compliant path for subscription logins.** Anthropic's legal/compliance doc states verbatim that OAuth is "intended exclusively for purchasers of Claude Free, Pro, Max, Team, and Enterprise subscription plans … Anthropic does not permit third-party developers to offer Claude.ai login into their own applications, or to route requests through Free, Pro, or Max plan credentials on behalf of their users. Moreover, developers may not collect, store, or intermediate Claude.ai credentials or session tokens — sign-in to a Claude account must complete through Anthropic's own flow." The same page adds, verbatim: "Nor does it prevent an end user from signing in to the unmodified Claude Code binary with their own Claude subscription, including where a platform hosts Claude Code as described under Can customers offer Claude Code in their products? above." — which is exactly this ADR's design and, per ADR-005's 2026-09-22 amendment, is now also shipped there as a chat-turn inference backend profile ("Anthropic via Claude Code / Agent SDK", `policy_status: allowed`), not only as an ADR-011 external coding agent ([Claude Code: Legal and compliance](https://code.claude.com/docs/en/legal-and-compliance), "Authentication and credential use", fetched 2026-09-22, via `providers-chat-auth-caching.md`). Gemini CLI's (now Antigravity CLI `agy`, D40) terms name a related pattern explicitly: "Directly accessing the services powering Gemini CLI (for example, the Gemini Code Assist service) using third-party software, tools, or services … is a violation of applicable terms and policies" ([Gemini CLI ToS & Privacy](https://geminicli.com/docs/resources/tos-privacy/), fetched 2026-09-22, ibid.) — whether that also covers the Anthropic-style "spawn the unmodified binary as a backend" shape is **unverified** for Google and is not assumed; ADR-005 carries it as an open re-check item. OpenAI's position on Codex OAuth in third-party harnesses is **ambiguous/unclarified** ([openai/codex Discussion #8338](https://github.com/openai/codex/discussions/8338), maintainer response 2025-12-19, ibid.). xAI's SuperGrok OAuth is undocumented and entitlement-gated (403s to legitimate subscribers, ibid.). Anthropic's policy in this exact area also moved three times within 2026 — an April ban of third-party subscription use, a 2026-05-13 announcement of a monthly "Agent SDK credit" covering this pattern, and a 2026-06-16 pause of that announcement with the statement "Nothing changes for now" ([the-decoder.com 2026-06-16](https://the-decoder.com/anthropic-backs-off-unpopular-billing-overhaul-as-price-war-with-openai-looms/); [support.claude.com/en/articles/15036540](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan), published 2026-06-16) — none of which changes the compliant shape (spawn the unmodified binary/SDK, never touch the credential), only what it costs the user.
...
| Gemini CLI (now Antigravity CLI `agy`, D40) | native `gemini --acp` | **Vendor-documented + called a reference implementation by the ACP project**, not third-party-validated | `docs/libraries/typescript.mdx:42` @ `bba7ddf`; registry 0.60.0; `protocols-channels-coding-clis.md` |
...
**Tier 1, shipped and in CI: Claude Code, OpenAI Codex CLI, Goose. Tier 1, shipped best-effort: Gemini CLI (now Antigravity CLI `agy`, D40). Tier 2 fallback: Claude Code and Codex. Everything else: discoverable and attachable, but labelled "community-tested, unverified" until a spike closes it.**
...
4. **Gemini CLI (now Antigravity CLI `agy`, D40) is native ACP and is called a production-ready reference implementation by the ACP project itself**, which makes it the cheapest fourth; but it is not third-party-cross-validated, so it ships best-effort rather than as an acceptance gate.
...
| **ToS / login reuse** | ADR-005 classifies a **harness-native** Claude.ai OAuth as **`prohibited`** (never shipped) but, as of the 2026-09-22 amendment, ships **"Anthropic via Claude Code / Agent SDK"** as `allowed` — spawning the unmodified binary/SDK under the user's own login; Google Gemini CLI (now Antigravity CLI `agy`, D40) / Code Assist / Antigravity OAuth stays **`prohibited`** (whether an analogous CLI-as-backend shape is permitted there is unverified, not assumed); OpenAI ChatGPT/Codex OAuth stays **ambiguous → not shipped**; xAI SuperGrok stays **unverified → not shipped** (ADR-005 policy table, sourced to vendor docs fetched 2026-09-22 via `providers-chat-auth-caching.md`, plus F2–F5 in `research/verification-log.md` V5–V8 for the Anthropic amendment) | **Running the vendor's own unmodified CLI is the compliant path** and is the whole design: the CLI authenticates itself, under its own ToS, with credentials we never see, read or transmit. We never implement a vendor's consumer OAuth flow, never store a subscription token, and never present one to a third-party client. Note Anthropic's companion clause "The Claude Code binary must not be modified" (ADR-005 policy table): we spawn `claude` unmodified, and `claude-agent-acp` wraps the **Claude Agent SDK**, not the CLI binary. Documented tokens the vendor publishes for programmatic use (`claude setup-token` → `CLAUDE_CODE_OAUTH_TOKEN`, `CODEX_API_KEY`) are the only ones we hold. Anthropic changed this exact policy three times in 2026 (April ban, May-13 credit announcement, June-16 pause), so treat any Anthropic announcement as an out-of-cycle re-check trigger, not just the 90-day cadence |
...
| 3 | Gemini CLI (now Antigravity CLI `agy`, D40) `--acp` end-to-end | pass/fail + notes | 0.5 d |
...
D5 names "claude-code, codex, gemini-cli, grok, kimi, opencode, pi, agy, goose, …" (gemini-cli is now Antigravity CLI `agy`, D40 — the brief's own list already separately names `agy`, so this is one CLI counted twice under old and new names). Only Claude Code, Codex and Goose are cross-validated end-to-end anywhere (`crates/buzz-acp/README.md:12` @ `77729ab`); the rest are vendor- or registry-listed only (`registry.mdx` @ `bba7ddf`).
...
**Recommended resolution:** (c). (a) and (b) are prohibited for at least two of the listed vendors and ambiguous for a third; ADR-005 already decides that no `prohibited` flow ships and names ADR-011 as the compliant route to the same user outcome. (c) satisfies D5's user-visible intent exactly — the user does not log in twice — while the harness never sees a secret. The discovery step is therefore restricted to *existence* checks, not reads. **2026-09-22 amendment:** for Anthropic this same shape — (c), spawning the unmodified binary/SDK, never touching its credential — is exactly what ADR-005 now ships as the "Anthropic via Claude Code / Agent SDK" profile, `policy_status: allowed`. That profile is a chat-turn inference backend (ADR-005), distinct from but built on the identical discovery/spawn contract this ADR defines for the same binary as an *external coding agent*; the two uses share one credential-never-touched rule and one discovery implementation. Nothing in this finding changes for Google, OpenAI or xAI: (c) is still the design, but no vendor has extended the Anthropic-style carve-out to them, and the Gemini case is explicitly carried as unverified rather than assumed to follow.
...
1. **Q7 (needs a decision now):** confirm the M6 set as Claude Code + Codex + Goose (gated) and Gemini CLI (now Antigravity CLI `agy`, D40) (best-effort)? Or add Copilot CLI, given Hermes already drives it?
...
6. For CLIs whose subscription login is **prohibited or ambiguous** for third-party use (Google, OpenAI, xAI — Anthropic is now `allowed` per the 2026-09-22 ADR-005 amendment and needs no such warning), should the harness still offer attachment (the vendor's own binary is compliant), with a warning — or hide them entirely?

...
| D12 | **"Also drei Wege: API, Restricted und Claude Code."** Anthropic ships three provider profiles, not one: **Claude Code (headless backend)** as the default (`policy_status: allowed`, spawns the unmodified `claude` binary/Agent SDK under the user's own login, harness never holds a credential); **setup-token** (OpenClaw-style, `policy_status: restricted`, opt-in with a visible risk notice, the harness stores a user-obtained `claude setup-token` token and calls the Messages API with Claude Code's own request shape); and the unchanged **API key** (`allowed`). This narrows original §6.3's no-client-imitation rule: *"Client-compatible request shaping is permitted only for tokens the user obtained themselves through the vendor's official flow; the harness never re-implements a vendor login, never reads another client's credential store, never refreshes another client's tokens."* The same decision moves **OpenAI ChatGPT/Codex OAuth and xAI device-code OAuth from "not shipped" to `restricted` opt-in with a risk notice**; **Google Gemini-CLI/Code Assist/Antigravity OAuth stays `prohibited`**, imported as a visible disabled profile, unless the 90-day re-check finds the ToS changed. Evidenced by the owner's own production OpenClaw 2026.9.5 installation running all four vendors under these exact modes (`research/verification-log.md` V9). | §6.1 provider table; §6.3 no-client-imitation rule (ADR-005 "Amendment D12", `provider-matrix.md` §4) |

Everything in the original not touched by D1–D12 remains binding: functional requirements §5–§9, platforms §10, security/quality §11, milestones §12 (to be re-cut for Variant B in `docs/milestones.md`), open questions §13 with their defaults.

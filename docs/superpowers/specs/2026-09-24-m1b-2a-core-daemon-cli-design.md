...
- **Engine key inventory.** A companion table `docs/config-engine-keys.md` (produced by the implementation plan's first documentation task) classifies each of the 56 engine keys as `live` or `core`; the harness cannot make a construction-time key live without an engine change. Which keys should become live is an engine follow-up (`mutateConfig` is the seam), out of 2a.
...
| E5 | Host-neutral `engine-config.schema.json` in the engine (the 56 keys with types, defaults, and a `readAt: construction|live` flag); the adapter translates for `openclaw.plugin.json`. | additive |
